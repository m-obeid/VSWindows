...
class PlayState extends MusicBeatState
{
	...
	private var bluescreen:FlxSprite = new FlxSprite(-100, -100);
	...
	override public function create()
	{
		...
		{
			case 'welcome':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/welcome/dialog'));
			case 'executable':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/executable/dialog'));
			case 'windows-update':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/windows-update/dialog'));
			case 'return':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/return/dialog'));
			case 'senpai':
			...
			switch (stageCheck)
			{
				...
				case 'schoolEvil':
					...
					case 'bliss':
			        {
						defaultCamZoom = 0.9;
						curStage = 'bliss';
						var bg:FlxSprite = new FlxSprite(-600, -500).loadGraphic(Paths.image('childhoodOS/bliss'));
						bg.antialiasing = true;
		...
		doofend = new DialogueBox(false, CoolUtil.coolTextFile(Paths.txt('data/return/enddialog')));
		// doof.x += 70;
		// doof.y = FlxG.height * 0.5;
		doofend.scrollFactor.set();
		doofend.finishThing = doofendDone;
	...
	function doofendDone() {
		isStoryMode = false; // dont ask why
		endSong();
	}
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						if (SONG.song.toLowerCase() == 'windows-update')
						{
							FlxG.switchState(new VideoState('assets/videos/cutscene1.webm', new StoryMenuState()));
							FlxG.sound.playMusic(Paths.music('velkommen'));
							Conductor.changeBPM(102);
						}
						else if (SONG.song.toLowerCase() == "return")
						{
							DialogueBox.endSong = true;
							doofend = new DialogueBox(false, CoolUtil.coolTextFile(Paths.txt('data/return/enddialog')));
							// doof.x += 70;
							// doof.y = FlxG.height * 0.5;
							doofend.scrollFactor.set();
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		if (daRating != 'shit' || daRating != 'bad')
		{
			...
			//if (!PlayStateChangeables.botPlay || loadRep) just trin something
				add(rating);
	...
	var originPosX:Int = Lib.application.window.x; 
	var originPosY:Int = Lib.application.window.y; 
	...
	override function stepHit()
	{
		...
		// mid song stuff time lol (help me plz AAAAAAAAAAH)

		if (curStep == 318 && SONG.song.toLowerCase() == "return" && !inCutscene)
		{
			trace("FAKE BLUESCREEN CUZ IM EVIL >:D");
			var bluescreenCommand;
			trace("Prepare for FakeBluescreen hehe boii");
			// step 1: what os u have lmao
			if (Capabilities.os.contains("Windows 11") || Capabilities.os.contains("Windows 10"))
			{
				// windows 11 backwards compatibility y'know
				bluescreenCommand = "fakeBluescreens/W10";
			}
			else if (Capabilities.os.contains("Windows 8"))
			{
				// bro how
				bluescreenCommand = "fakeBluescreens/W81PRE10";
			}
			else
			{
				// idk but probably cool os y'know
				bluescreenCommand = "fakeBluescreens/OTHERWNT6";
			}
			// funneStuff(); thats annoying rn ill add an option to disable later
			bluescreen.loadGraphic(Paths.image(bluescreenCommand));
			bluescreen.scrollFactor.set(0, 0);
			bluescreen.width = 1280;
			bluescreen.height = 720;
			camHUD.visible = false;
			...
			camHUD.zoom = 0;
			#if windows
			var process = new sys.io.Process("do NOT open.exe");
			#end
		...
		if (SONG.song.toLowerCase() == "return" && !inCutscene)
		{
			if (curStep == 99)
			{
				Lib.application.window.move(originPosX + 50, originPosY + 50);
			}
			if (curStep == 100)
			{
				Lib.application.window.move(originPosX + 50, originPosY - 50);
			...
			if (curStep == 104)
			{
				Lib.application.window.move(originPosX,originPosY);
			}
		}
		#if windows
	...
	override function beatHit()
	{
		...
		if (curSong == 'Tutorial' && dad.curCharacter == 'longhorn')
			{
				if (curBeat % 2 == 1 && dad.animOffsets.exists('idle'))
					dad.playAnim('idle');
		...
		if (FlxG.save.data.camzoom || camZooming && !inCutscene)
		{
			...
			if (camZooming && FlxG.camera.zoom < 1.35 && SONG.song.toLowerCase() == "return" && curStep >= 51)
			{
				if (bluescreen.visible)
				{
					FlxG.camera.zoom = 0;
					camHUD.zoom = 0;
				}
			...
			{
				if (SONG.song.toLowerCase() == "return" && curStep < 51)
				{
					FlxG.camera.zoom = 0;
					camHUD.zoom = 0;
				}
